...
def test_plotting():
    ...
    wavefront = make_optical_wavefront(
        field=jnp.array(field), wavelength=500e-9, dx=1e-6, z_position=0.0
    )
    ...
    wavefront_pol = make_optical_wavefront(
        field=jnp.array(field_pol), wavelength=500e-9, dx=1e-6, z_position=0.0
    )

    try:
        fig, axes = plot_complex_wavefront(
            wavefront_pol, title="Polarized Complex"
        )
        plt.close(fig)
